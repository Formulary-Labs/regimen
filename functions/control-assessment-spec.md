...
structured_output_schema: config/schemas/control-assessment-state.schema.json
...
**Schema:** `config/schemas/control-assessment-state.schema.json`. Load that file to populate the output; do not reconstruct it from memory of a prior version.

Top-level shape: `run_id`, `program`, `framework`, `product_name`, `product_version`, `template_path`, `product_source`, `started`, `last_updated`, `status`, `template_type`, `field_map`, `control_inventory` (per-control status and batch assignment), `product_index`, `batches` (each with `control_ids`, `status`, and `responses` keyed by control ID — `citation`, `narrative`, `determination`, `gap`, `citation_confidence`), `summary` (aggregate counts).

---

## Formulary Tool

**CLI:** [`assay`](https://github.com/Formulary-Labs/assay) — `github.com/Formulary-Labs/assay`

Handles the deterministic assessment orchestration layer:
- Initializes assessment state from a gemara `ControlCatalog` artifact
- Builds configurable batch sizes and tracks control status per batch
- Validates responses against 7 criteria (present, specific, auditable, dated, owner, evidence linked, no fabrication signals)
- Resumes from last validated control — safe to interrupt and restart

```bash
assay init --catalog data/[PROGRAM]/control-catalog.yaml --state data/[PROGRAM]/assessments/[RUN_ID]-state.json
assay batch --state data/[PROGRAM]/assessments/[RUN_ID]-state.json --size 10
assay validate --state ... --responses responses.json
assay assemble --state ... --out data/[PROGRAM]/assessments/[RUN_ID]-report.md
assay status --state ...
```

This spec governs the full assessment workflow: framework doc loading, product documentation indexing (Phase 1), template analysis (Phase 2), narrative response generation — LLM task (Phase 3), cross-control consistency pass (Phase 4), output assembly (Phase 5), and quality gate (Phase 6). `assay` handles the state machine, batch construction, and 7-criterion validation.
