...
**CLI:** [`specimen`](https://github.com/Formulary-Labs/specimen) — `github.com/Formulary-Labs/specimen`

Handles the deterministic risk register and POA&M data layer:
- Add, list, and filter risk entries with severity, owner, and due date
- Ingest post-audit feed-forward artifacts directly into the register
- Status summary and structured JSON output for pipeline consumption

```bash
specimen add --register data/[PROGRAM]/risk-register.json --severity High --control AC-1 --owner "Jane Smith" --due 2026-12-01
specimen list --register ... --filter open
specimen ingest --register ... --feed-forward data/[PROGRAM]/post-audit/2026-ff.json
specimen status --register ...
```

This spec governs risk scoring judgment, owner assignment, remediation classification, pipeline write to `runs/[PROGRAM]/latest.json`, POA&M formatting, and integration with monitoring and auditor view. `specimen` handles the data layer; this spec handles AI judgment and routing.
