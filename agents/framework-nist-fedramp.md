...
Session input: `FRAMEWORK_QUERY: <query about framework controls or requirements>`

The agent loads the framework control catalog, guidance materials, and active program assessments. Interprets the query against the framework. Returns control interpretation, applicability guidance, or documentation language. All output is flagged for lead PM review before adoption.

### Deployed Mode

The agent listens on the `framework-channel` message bus for `FRAMEWORK_QUERY` messages routed by the program coordinator. Emits `CONTROL_MAPPING` or framework-specific responses tagged with `program_id` for async consumption by program agents. All analysis is logged to provenance for compliance artifact traceability.
