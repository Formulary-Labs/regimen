...
## State Writes
- `logs/provenance.jsonl` — append review entries and, per the approval workflow below, spec improvement entries
- Spec files named in an approved (`yes`/`edit`) spec improvement — never on `no`, never without the interactive approval step
...
This spec implements the D1-D6 review function owned by `agents/review-agent.md`. Its Authority Boundary applies here in full — this section states only what is specific to run-reviewer's own D6/approval-workflow mechanics, not a restatement of that agent's general review authority.

### Autonomous (no lead program manager approval)
- Execute D1-D5 against any completed run and produce the verdict and findings
- Draft D6 spec improvement proposals for staging

### Escalate to Lead program manager
- Every D6 spec improvement — presented one at a time per the Approval Workflow above; `yes`/`edit` writes immediately, `no` is logged as declined
...
- REQUIRES CORRECTION verdicts

### Never
- Write to a spec file without an explicit `yes` or `edit` response to that specific proposal
- Batch-apply multiple spec improvements without individual confirmation
- Soften a verdict to avoid presenting an uncomfortable D6 proposal

## Communication Interface

Messaging is owned by `agents/review-agent.md` (`REVIEW_REQUEST`/`REVIEW_COMPLETE`, `CRITICAL_FINDING`, `SPEC_IMPROVEMENT`) — this spec is the procedure that agent executes for post-run review specifically. No separate message types are defined here.

## Instantiation

### IDE Mode
- Invoked per the routing table (`engine/session-init-spec.md`) after a pipeline run, control assessment run, or manual update batch, or on demand for audit prep
- D6 approval workflow runs interactively — each proposal presented and confirmed before the next
- Output presented directly to the lead program manager per the Output format above

### Deployed Mode
- Runs as `agents/review-agent.md`'s `REVIEW_REQUEST` handler
- D6 proposals queued to the decision gate API rather than presented interactively; spec writes wait for the queued approval to resolve before proceeding
- `REVIEW_COMPLETE` emitted with the verdict once D1-D5 finish, independent of whether D6 proposals are still pending approval
