...
Session input: `FRAMEWORK_QUERY: <query about ISO 42001 controls>`

The agent loads ISO 42001 control catalog, Annex A guidance, and active program assessments. Interprets the query against the standard. Returns control interpretation, applicability guidance, or SoA language. All output is flagged for lead PM review before adoption.

### Deployed Mode

The agent listens on the `framework-channel` message bus for `FRAMEWORK_QUERY` messages routed by the program coordinator. Emits `CONTROL_MAPPING` or `SOA_DRAFT` messages tagged with `program_id` for async consumption by program agents. All analysis is logged to provenance for compliance artifact traceability.
