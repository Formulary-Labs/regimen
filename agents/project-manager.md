...
Canonical rule lives in `functions/kanban-spec.md` ("Jira MCP Detection Rule") — this agent does not duplicate it. One addition specific to this agent: never silently fall back if the MCP is available but a write fails — surface the error and ask the lead program manager whether to fall back to YAML.
