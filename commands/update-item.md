...
Update the status, owner, or notes on a specific POA&M item, control gap, or action item identified by its GRC ID.
...
2. Search for the GRC ID across: `risk_register.items`, `control_coverage.gaps`, `poam.items`
3. Display the current item before making any change:
...
4. Confirm the update with the lead program manager before writing
5. Apply the update to the relevant field in `runs/[program]/latest.json`
6. Append a timestamped note to the item's `notes` array if present:
