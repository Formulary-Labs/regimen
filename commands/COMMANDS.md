...
| `/portfolio-qbr` | Quarterly narrative synthesis across all active programs | none (optional: period, program filter) |
| `/doc-review` | Two-tier AI-writing-tell and format scan on a document or passage | target (file path or pasted text) |
| `/post-audit` | Lessons learned, corrective actions, and feed-forward from a closed audit | program, audit type, audit cycle |
| `/validate` | Run the system's structural validators (frontmatter, schema drift, spec coverage, integrity, script safety) in one pass | none (optional: program scope) |
...
### `/portfolio-qbr`
Produces a quarterly business review synthesizing narrative intelligence across all active programs — cross-program patterns, trend analysis, and strategic recommendations not visible from any single program view. Reads all memory, decision logs, run JSONs, portfolio state, and provenance log.

```
/portfolio-qbr
/portfolio-qbr PERIOD: 2026-04-01 to 2026-06-30
```

Arguments (all optional, sensible defaults):
- `PERIOD` — defaults to the last 90 days
- `PROGRAMS` — defaults to all active programs
- `INCLUDE_PRIOR_QBR` — defaults to yes if a prior QBR exists

Output: `data/portfolio/qbr/[date]-qbr.md` and `data/portfolio/qbr/latest.md`. Requires two independent program signals minimum before naming a cross-program theme — never infers a pattern from a single program's data.

---

### `/doc-review [target]`
Two-tier scan for AI writing patterns that undermine credibility, neutrality, and evidentiary precision in compliance artifacts, per `engine/doc-style-guide.md`. Read-only — surfaces findings, does not rewrite the document.

```
/doc-review data/iso42001/drafts/status-update.md
/doc-review [pasted passage]
```

Arguments:
- `target` — a file path, or pasted text to scan directly
- `scope` (optional, default: full) — `tier1` for critical patterns only

Tier 1 findings are blocking (REJECT + correction brief). Tier 2 flags are advisory. If the target already passed the quality gate, a Tier 1 finding here is a retroactive gate credibility failure — flagged explicitly.

---

### `/post-audit [program]`
Processes audit closure into structured lessons learned, corrective action plans, and a feed-forward artifact for the next program cycle, per `functions/post-audit-spec.md`.

```
/post-audit fedramp-high
```

Arguments (prompted if not supplied):
- `program` — program slug
- `audit type` — `external` `surveillance` `internal`
- `audit cycle` — `YYYY` or a date range
- `mode` — `standard` (default) or `full_retrospective` (adds program improvement items, feed-forward artifact, draft closure communication)

Output: `data/[program]/post-audit/[AUDIT_CYCLE]-*`. The feed-forward artifact (full_retrospective only) is consumed by the next `program-intake-spec.md` run for this program.

---

### `/validate`
Runs the system's structural validators in one pass: `integrity_check.py`, `validate_frontmatter.py`, `validate_schema_drift.py`, `spec_coverage.py`, `validate_script_safety.py`, plus any program-specific validator (e.g. `validate_aims_xlsx.py` for `iso42001`). A repo-health check, not a compliance-content check.
...
Arguments:
- `scope` (optional) — `system` (default) or a program slug to add that program's own validator

Reports pass/fail per validator with actual error output on failure. Does not auto-fix anything found.
...
commands/kanban.md
commands/portfolio-qbr.md
commands/doc-review.md
commands/post-audit.md
...
.cursor/skills/portfolio-qbr/SKILL.md
.cursor/skills/doc-review/SKILL.md
.cursor/skills/post-audit/SKILL.md
