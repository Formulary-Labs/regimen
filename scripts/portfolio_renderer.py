...
DEBUG_LOG_PATH = Path("/Users/alangsto/Documents/Portfolio/.cursor/debug-1ea027.log")


def _debug_log(run_id: str, hypothesis_id: str, location: str,
...
def _lookup_program_name(slug: str, runs_dir: Path) -> str:
    ...
    _debug_log(
        run_id="pre-fix",
        hypothesis_id="H2",
        location="portfolio_renderer.py:_lookup_program_name",
        message="Program-name lookup path",
        data={"slug": slug, "path": str(path), "exists": path.exists()},
    )
    # endregion
    if path.exists():
        try:
...
def render_program_cards(portfolio: dict, public_dir: Path) -> str:
    ...
    _debug_log(
        run_id="pre-fix",
        hypothesis_id="H1",
        location="portfolio_renderer.py:render_program_cards:start",
        message="Program list shape",
        data={
            "program_count": len(programs),
            "first_program_keys": sorted(list(programs[0].keys()))[:20] if programs else [],
        },
    )
    # endregion
    for prog in programs:
        ...
        _debug_log(
            run_id="pre-fix",
            hypothesis_id="H1",
            location="portfolio_renderer.py:render_program_cards:raw",
            message="Raw program key variants",
            data={
                "program_slug": prog.get("program_slug"),
                "slug": prog.get("slug"),
                "display_name": prog.get("display_name"),
                "frameworks": prog.get("frameworks"),
                "framework": prog.get("framework"),
            },
        )
        # endregion
        ...
        _debug_log(
            run_id="pre-fix",
            hypothesis_id="H3",
            location="portfolio_renderer.py:render_program_cards:derived",
            message="Derived render values",
            data={
                "derived_slug": slug,
                "derived_name": name,
                "health": health,
