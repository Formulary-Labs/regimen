...
def validate_spec(
    fm: dict[str, Any], rel_path: str, repo_root: Path | None = None
) -> list[str]:
    ...
    if fm.get("structured_output") is True:
        schema_ref = fm.get("structured_output_schema")
        if not schema_ref:
            findings.append(
                "structured_output: true requires structured_output_schema"
            )
        elif not str(schema_ref).startswith("config/schemas/"):
            findings.append(
                f'structured_output_schema "{schema_ref}" should live under '
                f"config/schemas/"
            )
        elif repo_root is not None and not (repo_root / str(schema_ref)).exists():
            findings.append(
                f'structured_output_schema "{schema_ref}" does not exist'
            )
...
def main() -> int:
    ...
    for fpath, ftype in files:
        ...
        if ftype == "agent":
            ...
            findings = validate_spec(fm, rel, repo_root)
