...
**Trigger:** Any markdown heading (`#`, `##`, `###`, etc.) that begins with a numeric prefix (e.g., `## 1. Purpose`, `### 3.1 Inherent Risk Scores`).

Markdown heading hierarchy provides sufficient structural ordering. Numeric prefixes duplicate what the heading level already conveys, create maintenance burden when sections are reordered, and produce inconsistent numbering when documents are assembled from multiple sources.

**FLAG when** any heading in a compliance document begins with a digit followed by a period or dot-separated sub-number.

**Required fix:** Remove the numeric prefix. `## 1. Purpose` becomes `## Purpose`. `### 3.1 Inherent Risk Scores` becomes `### Inherent Risk Scores`. ISO clause references (e.g., `### Clause 4.1`) are not numeric prefixes — they are part of the heading's semantic content and are permitted.

### Horizontal Rule Section Dividers

**Trigger:** A horizontal rule (`---`, `***`, or `___`) used as a visual separator between sections in a compliance document.

Markdown headings provide sufficient visual and structural separation between sections. Horizontal rules add visual noise without conveying information and are inconsistently rendered across export formats (PDF, HTML, Confluence).

**FLAG when** a horizontal rule appears between sections in a compliance document. YAML frontmatter delimiters (`---` at lines 1 and at the end of frontmatter) are not section dividers and are not flagged.

**Required fix:** Remove the horizontal rule. Use a single blank line between sections. Heading hierarchy provides the structural break.
