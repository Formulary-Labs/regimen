...
Core rules (classify before acting, one-way door confirmation, quality gate, provenance reuse check, recursive search before flagging missing) are governed by `.cursor/rules/identity.mdc` / `CLAUDE.md` — already loaded, not restated here.

Session-init-specific: never load a spec, memory file, or run JSON before classification completes (Step 1 discovery only); always use live directory discovery, never assume a file exists.
