...
structured_output_schema: config/schemas/qbr-output.schema.json
depends_on:
  - memory/*-memory.md
...
OUTPUT_FORMAT:      [markdown | json | both — default markdown. json emits the structure declared in config/schemas/qbr-output.schema.json alongside/instead of the prose report; it must pass the same quality gate as the markdown form, not a relaxed one]
```
