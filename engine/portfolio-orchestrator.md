...
depends_on:
  - runs/*/latest.json
  - memory/*-memory.md
  - data/portfolio/trust-state.json
structured_output: true
structured_output_schema: config/schemas/portfolio-state.schema.json
---
...
**Staleness auto-remediation (Path 2, scoped):** If a program's *only* Red/Yellow trigger is staleness (past `next_run_recommendation`, no escalation/blocker/decision-age trigger also firing), check `data/portfolio/trust-state.json`'s `monitoring_run` trust level for `portfolio-coordinator` before adding it to Suggested First Actions. At trust level 1 (default), surface it as a suggested action as before. At level 2+, invoke `engine/program-pipeline-orchestrator.md` (`INTENT: monitoring_run`) directly, narrate the auto-initiation, and report the result in the briefing instead of asking. Full policy: `agents/coordinator.md` § Path 2 Policy.
...
**Schema:** `config/schemas/portfolio-state.schema.json`. Load that file to populate the output; do not reconstruct it from memory of a prior version.

**Canonical field names (use these — do not use the aliases):** `program_slug` (not `slug`), `health_reason`, `last_run_date` (not `last_run`), `frameworks` (not `framework`), `next_run_recommended`. The schema also accepts a legacy alias set (`slug`, `summary`, `framework`, `cross_program`) for backward compatibility with older renders — new writes must use the canonical names only, to avoid perpetuating the field-naming drift this schema was patched to tolerate.

Top-level shape: `schema_version`, `generated_at`, `generated_by`, `programs` (array of program summaries), `cross_program_signals`, `portfolio_health`, `suggested_actions` (max 5), `notes`.
