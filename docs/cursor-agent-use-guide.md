...
### Multi-PM teams (shared repo)

If several program managers use the same clone: declare **your** identity in Cursor **User rules** and keep personal scratch under `local/pm/<your-handle>/` (gitignored). Canonical roster and copy-paste templates: **`config/pm-roster.md`** and **`docs/multi-pm-cursor.md`**. In Agent chat, request the **`pm-cohort`** workspace rule when onboarding or when edits touch `memory/` / `runs/` for a named owner.
